...
function getSupabase(){
  if(!supabaseClient){
    if(!window.supabase || !window.supabase.createClient) throw new Error("Supabase não carregou no navegador.");
...
function toDb(x){return {client:x.client,phone:x.phone,insurer:x.insurer,branch:x.branch,value:Number(x.value||0),end_date:x.endDate||null,claim:x.claim||"Não",status:x.status||"Ativo",notes:x.note||"",last_renewal_year:x.lastRenewalYear==null?null:Number(x.lastRenewalYear)}}
async function loadFromSupabase(){
  const {data:rows,error}=await getSupabase().from("Seguros").select("*").order("created_at",{ascending:false});
...
async function insertRecord(x){const {error}=await getSupabase().from("Seguros").insert(toDb(x));if(error)throw error}
async function updateRecord(x){const {error}=await getSupabase().from("Seguros").update(toDb(x)).eq("id",Number(x.id));if(error)throw error}
async function deleteRecord(id){const {error}=await getSupabase().from("Seguros").delete().eq("id",Number(id));if(error)throw error}
...
const SESSION_KEY="pg_seguros_login_expira";
const SESSION_MS=3*60*60*1000;
function enterApp(){$("login").classList.add("hidden");$("app").classList.remove("hidden");showPage("dashboard");loadFromSupabase().catch(err=>console.error(err))}
function hasValidSession(){const exp=Number(localStorage.getItem(SESSION_KEY)||0);if(exp>Date.now())return true;localStorage.removeItem(SESSION_KEY);return false}
function startSession(){localStorage.setItem(SESSION_KEY,String(Date.now()+SESSION_MS));}
...
$("loginForm").addEventListener("submit",e=>{e.preventDefault();if($("username").value==="pgseguros"&&$("password").value==="PgBc@2027"){startSession();enterApp()}else $("loginError").textContent="Usuário ou senha incorretos."});
$("logout").onclick=()=>{clearSession();$("app").classList.add("hidden");$("login").classList.remove("hidden");$("password").value=""};
if(hasValidSession())enterApp();
...
function importRow(row){
 ...
 const value=parseMoney(pick(row,["Prêmio Anterior","Premio Anterior","Valor do seguro","Valor","Premio","Prêmio","Premio Total","Prêmio Total","Valor Total","Premio Liquido","Prêmio Líquido"]));
 const endDate=excelDate(pick(row,["FINAL VIGÊNCIA","Final Vigência","Final da vigência","Final da vigencia","Fim da vigência","Fim da vigencia","Data Final","Data Fim","Vigencia Final","Vencimento"]));
 return {client,phone,insurer,branch,value,startDate:"",endDate,note:""};
}
...
$("confirmImport")?.addEventListener("click",async()=>{
 ...
 try{
  const importPayload=importRows.map(x=>({
    client:x.client,
    phone:x.phone||null,
  ...
  const {error}=await getSupabase().from("Seguros").insert(importPayload);
  if(error)throw error;
  await refresh(); closeImport(); showPage("seguros"); alert(`${importPayload.length} cliente(s) importado(s) com sucesso!`);
 }catch(err){console.error(err);alert("Não foi possível importar para o Supabase: "+(err?.message||err))}
