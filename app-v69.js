...
$("loginForm").addEventListener("submit",e=>{e.preventDefault();if($("username").value==="pgseguros"&&$("password").value==="PgBc@2027"){$("login").classList.add("hidden");$("app").classList.remove("hidden");showPage("dashboard");loadFromSupabase().catch(err=>console.error(err))}else $("loginError").textContent="Usuário ou senha incorretos."});
